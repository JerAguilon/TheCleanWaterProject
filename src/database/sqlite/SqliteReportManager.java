...
class SqliteReportManager extends Connectable {
    ...
    public Collection<WorkerReport> getWorkerReportList() throws SQLException {
        List<WorkerReport> output = new ArrayList<>();
    ...
    public boolean addWorkerReport(Report report) {
        try {
            if (report instanceof WorkerReport) {
                addWorkerReport((WorkerReport) report);
                return true;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private void addWorkerReport(WorkerReport report) throws SQLException {
        ...
        PreparedStatement preparedStatement = null;
        String query = "INSERT INTO userReports "
                + "(creationDate, username, location, condition) VALUES"
                + "(?,?,?,?,?)";
        try {
            preparedStatement = connection.prepareStatement(query);
